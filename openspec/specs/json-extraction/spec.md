...
Obtain structured JSON from LLM responses reliably, by using the OpenAI-compatible structured-output API (`ChatOpenAI.with_structured_output()`) so the API itself enforces valid JSON — rather than relying on fragile client-side regex parsing. A regex fallback covers models that don't support structured output. This is the single mechanism behind `OpenRouterClient.extract_json()`, used by all structured pipeline steps (brief sections, LSI keywords, claim extraction, fact verification).
