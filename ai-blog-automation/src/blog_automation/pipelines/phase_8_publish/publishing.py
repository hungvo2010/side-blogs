...
from blog_automation.errors import PublishingFailureError
...
from blog_automation.pipelines.phase_6_quality.quality_gates import run_quality_gates
...
def _add_gutenberg_blocks(html: str) -> str:
    ...
    for level in range(2, 7):
        html = re.sub(
            rf"<h{level}([^>]*)>(.*?)</h{level}>",
            rf'<!-- wp:heading {{"level":{level}}} -->\n<h{level}\1>\2</h{level}>\n<!-- /wp:heading -->',  # noqa: E501  long regex replacement with backrefs; keep intact
            html,
            flags=re.DOTALL,
        )
    ...
    html = re.sub(
        r"<pre><code[^>]*>(.*?)</code></pre>",
        r"<!-- wp:code -->\n<pre class=\"wp-block-code\"><code>\1</code></pre>\n<!-- /wp:code -->",  # noqa: E501  long regex replacement with backrefs; keep intact
        html,
        flags=re.DOTALL,
    )
...
def publish_article(
    ...
    try:
        with get_session() as session:
            ...
            # Setup analytics
            setup_analytics_tracking(article)
