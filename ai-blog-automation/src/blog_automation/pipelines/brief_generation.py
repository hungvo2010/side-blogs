...
from blog_automation.pipelines.keyword_research import research_keyword

logger = get_logger(__name__)


# Prompts for brief generation
SECTION_GENERATION_PROMPT = """Based on this keyword research, suggest H2 sections for a comprehensive blog post.
...
LSI_KEYWORD_PROMPT = """Generate LSI (Latent Semantic Indexing) keywords related to the main keyword.
...
UNIQUE_ANGLE_PROMPT = """Analyze these competitor articles and suggest a unique angle for our content.
    ...
    try:
        ...
        with get_session() as session:
            ...
            unique_angle = _generate_unique_angle(
                claude, keyword, intent, brief_data.get("competitor_analysis", {})
            )
...
def _generate_audience_description(keyword: str, intent: str) -> str:
    ...
    """
    intent_audiences = {
        "informational": f"People seeking to learn about {keyword}, including beginners and those looking to deepen their understanding.",
        "commercial": f"Potential buyers researching {keyword} options, comparing features and looking for recommendations.",
        "transactional": f"Ready-to-buy customers looking for the best deals on {keyword}.",
