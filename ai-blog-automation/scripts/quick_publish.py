...
def run(kw):
    ...
    print(f"4. ✅ Fact check done")
    seo_optimize_article(article)
    print(f"5. ✅ SEO: score={article.seo_score}")
    run_quality_gates(article)
    print(f"6. ✅ Quality done")
    result = publish_article(title=article.title or kw, content=article.content_draft or "", keyword=kw, auto_push=True)
