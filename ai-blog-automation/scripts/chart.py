...
CARD = "#161b22"
BORDER = "#30363d"
COLORS = ["#58a6ff", "#f78166", "#d2a8ff", "#7ee787", "#f0883e",
          "#a5d6ff", "#ff7b72", "#d2a8ff", "#56d364", "#db6d28"]
WHITE = "#c9d1d9"
MUTED = "#8b949e"
GREEN = "#3fb950"
RED = "#f85149"
...
for kw in keywords:
    try:
        ...
        d = hash(kw * 2) % 70 + 15
        metrics[kw] = {"vol": v, "diff": d, "interest": 0,
                        "score": v * (100 - d) / 100}
...
try:
    ...
    for i, kw in enumerate(keywords):
        if kw in df.columns:
            ax1.plot(df.index, df[kw], label=kw,
...
ax1.legend(facecolor=CARD, edgecolor=BORDER, labelcolor=WHITE, fontsize=10,
           loc="upper left")
...
for kw, m in metrics.items():
    score = m["score"]
    if score > 50000: verdict, vc = "HOT", GREEN
    elif score > 10000: verdict, vc = "Good", GREEN
    elif score > 2000: verdict, vc = "OK", YELLOW
    else: verdict, vc = "Low", RED
...
for key, cell in table.get_celld().items():
    ...
    if key[0] == 0:  # header
        ...
        if key[1] == 4:  # verdict column
            ...
            if "HOT" in v: cell.set_text_props(color=GREEN)
            elif "Good" in v: cell.set_text_props(color=GREEN)
            elif "OK" in v: cell.set_text_props(color=YELLOW)
            else: cell.set_text_props(color=RED)

# Watermark
fig.text(0.5, 0.01, f"Generated {datetime.now().strftime('%Y-%m-%d %H:%M')}  ·  pytrends",
